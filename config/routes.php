...
return function (ContainerInterface $c): Router {
    ...
    // === MARK AUTH ROUTES ===
    $router->get('/mark/login', 'mark.login.form', fn($req) => $c->get(AuthController::class)->loginForm($req));
    $router->post('/mark/login', 'mark.login', fn($req) => $c->get(AuthController::class)->login($req));
    $router->get('/mark/logout', 'mark.logout', fn($req) => $c->get(AuthController::class)->logout($req));
    ...
    // === MARK DASHBOARD API ROUTES ===
    $router->get('/api/mark/dashboard/stats', 'api.mark.dashboard.stats', fn($req) => $c->get(\Blog\Infrastructure\Http\Controller\Api\Mark\DashboardApiController::class)->getStats($req));
    $router->get('/api/mark/audit-logs/recent', 'api.mark.audit-logs.recent', fn($req) => $c->get(\Blog\Infrastructure\Http\Controller\Api\Mark\AuditLogApiController::class)->getRecent($req));
    $router->get('/api/mark/articles/recent', 'api.mark.articles.recent', fn($req) => $c->get(\Blog\Infrastructure\Http\Controller\Api\Mark\ArticlesApiController::class)->getRecent($req));
    $router->get('/api/mark/users/recent', 'api.mark.users.recent', fn($req) => $c->get(\Blog\Infrastructure\Http\Controller\Api\Mark\UsersApiController::class)->getRecent($req));
